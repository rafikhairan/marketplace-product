<main class="w-full md:w-[calc(100%-256px)] md:ml-64 bg-gray-50 min-h-screen transition-all main">
  ...
  <div class="p-6">
    <h3 class="font-semibold text-2xl text-gray-900 mb-6">Edit Product</h3>
    <form action="<?= BASEURL . "/dashboard/updateProduct/" . $data['product']['id']; ?>" method="post" enctype="multipart/form-data">
      ...
      <div class="grid grid-cols-2 gap-x-6">
        <div class="mb-6">
          ...
          <input type="text" id="product-name" name="product_name" class="bg-gray-50 border border-gray-300 text-gray-500 text-sm rounded-lg focus:ring-0 focus:outline-none focus:border-gray-300 block w-full p-2.5" placeholder="Product name here" value="<?= $data['product']['product_name']; ?>" required>
        </div>
        <div class="mb-6">
          ...
          <select id="category" name="category" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-0 focus:outline-none focus:border-gray-300 block w-full p-2.5" required>
            <?php foreach($data['categories'] as $category): ?>
              <?php if($category === $data['product']['category_name']): ?>
        ...
        <div class="mb-6">
          ...
          <input type="number" id="stock" name="stock" class="bg-gray-50 border border-gray-300 text-gray-500 text-sm rounded-lg focus:ring-0 focus:outline-none focus:border-gray-300 block w-full p-2.5" min="1" max="99" value="<?= $data['product']['stock']; ?>" required>
        </div>
        <div class="mb-6">
          ...
          <div class="flex">
            ...
            <input type="text" id="price" name="price" class="rounded-none rounded-r-lg bg-gray-50 border text-gray-500 focus:ring-0 focus:outline-none focus:border-gray-300 block flex-1 min-w-0 w-full text-sm border-gray-300 p-2.5" value="<?= $data['product']['price']; ?>" required>
          </div>
        </div>
      </div>
      <div class="mb-6">
      ...
      <div class="mb-6">
        ...
        <textarea id="description" name="description" rows="6" class="block p-2.5 w-full text-sm text-gray-500 bg-gray-50 rounded-lg border border-gray-300 focus:ring-0 focus:outline-none focus:border-gray-300" placeholder="Product description here" required><?= $data['product']['description']; ?></textarea>
      </div>
